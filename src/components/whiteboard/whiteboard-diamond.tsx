...
export const WhiteboardDiamond = ({ item, onUpdate, onDelete }: Props) => {
  ...
  const onResizing = (e: MouseEvent) => {
    ...
    const nextWidth = Math.max(60, startRef.current.width + dx)
      ...
      <div ref={nodeRef} className="absolute cursor-move" style={{ width, height }}>
        ...
        >
          <polygon
            ...
            className={cn('transition-all hover:scale-105', shapeColors[(item.shapeColor as ShapeColorKey) || 'pink'])}
            strokeWidth={2}
