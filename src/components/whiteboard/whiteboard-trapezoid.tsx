...
export const WhiteboardTrapezoid = ({ item, onUpdate, onDelete }: Props) => {
  ...
  const onResizing = (e: MouseEvent) => {
    ...
    const nextWidth = Math.max(80, startRef.current.width + dx)
    const nextHeight = Math.max(50, startRef.current.height + dy)
      ...
      <div ref={nodeRef} className="absolute cursor-move" style={{ width, height }}>
        ...
        >
          <polygon
            ...
            className={cn('transition-all hover:scale-105', shapeColors[(item.shapeColor as ShapeColorKey) || 'orange'])}
            strokeWidth={2}
