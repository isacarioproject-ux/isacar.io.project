...
export const WhiteboardPentagon = ({ item, onUpdate, onDelete }: Props) => {
  ...
  const onResizing = (e: MouseEvent) => {
    ...
    const nextWidth = Math.max(70, startRef.current.width + dx)
    const nextHeight = Math.max(60, startRef.current.height + dy)
    ...
    >
      <div ref={nodeRef} className="absolute cursor-move" style={{ width, height }}>
        ...
        >
          <polygon
            points={points}
            className={cn('transition-all hover:scale-105', shapeColors[(item.shapeColor as ShapeColorKey) || 'purple'])}
            strokeWidth={2}
